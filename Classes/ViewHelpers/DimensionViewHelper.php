...
class DimensionViewHelper extends AbstractViewHelper
{
    ...
    public function render()
    {
        ...
        for($i=0; $i < $maxValue; $i++) {
            ...
            $description = "";
            $nextNoX = false;
            if(isset($descriptions[$i]) && $descriptions[$i] != '␣') {
                $nextNoX = true;
                $description = $descriptions[$i];
            }
            if(isset($diameters[$i]) && $diameters[$i] != '␣') {
                if($nextNoX) {
