            ...
            if (!empty($array[$queryField['id']])) {
                if (is_array($array[$queryField['id']])) {
                    ...
                    for ($i=0;$i < sizeof($array[$queryField['id']]);$i++) {
                        $replacement = (string)($array[$queryField['id']][$i] ?? '');
                        $resultValue = str_replace('%' . ($i + 1) . '$s', $replacement, $resultValue);

                    }
                } else {
                    $replacement = (string)$array[$queryField['id']];
                    $resultValue = str_replace('%1$s', $replacement, $queryField['query']);
                }
            }
        }
